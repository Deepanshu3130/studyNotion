                    ...
                    {/* Cards shure hote h ab */}
                    {
                        enrolledCourses.map((course,index)=> (
                            <div key={index} onClick={()=>{
                                navigate(`view-course/${course._id}/section/${course.courseContent[0]._id}/sub-section/${course.courseContent[0].subSection[0]}/title/${course.courseName}`)}}
                                ...
                                <div className='flex w-1/5 flex-col gap-2 px- py-3'>
  {
    ...
    progressData?.length > 0
      ? progressData.map((progress, index) => {
        // Show progress only for the current course
        if (progress?.courseID === course?._id) {
          ...
          const progressPercentage = totalLectures > 0 ? (completedVideos / totalLectures) * 100 : 0;

          return (
            <div key={index}>
              <p>Completed: {completedVideos} / {totalLectures}</p>
              <ProgressBar
      ...
      : (
        // If no progress data is available, show 0% progress for the current course
        <div>
